...
public class TcpMessageReceiver extends AbstractMessageReceiver implements Work {
	...
	protected class TcpWorker implements Work, Disposable {
		...
		protected byte[] processData(byte[] data) throws Exception {
			...
			ArrayList messages = loadHL7Messages(str_data);
		...
		private ArrayList<String> loadHL7Messages(String incomingMessages)
				throws FileNotFoundException {
			ArrayList<String> messages = new ArrayList<String>();
			StringBuilder message = new StringBuilder();
			Scanner scanner = new Scanner(incomingMessages);
			char data[] = { (char) startOfMessage, (char) endOfMessage };

			while (scanner.hasNextLine()) {
				String line = scanner.nextLine().replaceAll(
						new String(data, 0, 1), "").replaceAll(
						new String(data, 1, 1), "");
