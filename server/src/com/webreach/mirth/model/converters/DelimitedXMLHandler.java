	...
	public void characters(char ch[], int start, int length) {
		...
		if (inColumn) {
			...
			if (props.getColumnWidths() != null) {
				...
				String columnValue = new StringBuilder().append(ch, start, length).toString();
				
				// If the column value contains the column delimiter, or record delimiter
				if (columnValue.contains(getColumnDelimiter()) || columnValue.contains(getRecordDelimiter())) {
					
					// Escape the escape characters and the quote characters
					columnValue.replaceAll(getQuoteEscapeChar(), getEscapedQuoteEscape());
					columnValue.replaceAll(getQuoteChar(), getEscapedQuote());
